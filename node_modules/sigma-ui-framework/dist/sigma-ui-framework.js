define(["require", "exports", "./utils/ui-constants", "./utils/ui-utils", "./utils/ui-validation", "aurelia-validation", "./utils/ui-event", "./utils/ui-constants", "./utils/ui-formatters", "./utils/ui-application", "./utils/ui-model", "./utils/ui-dialog", "./utils/ui-tree-models", "./utils/ui-http-service", "./utils/ui-validation", "./utils/ui-utils", 'lodash', 'moment', 'numeral', 'kramed', './scripts/phonelib', './scripts/fileTypes', './scripts/countries', './scripts/currencies'], function(require, exports, ui_constants_1, ui_utils_1, ui_validation_1, aurelia_validation_1, ui_event_1, ui_constants_2, ui_formatters_1, ui_application_1, ui_model_1, ui_dialog_1, ui_tree_models_1, ui_http_service_1, ui_validation_2, ui_utils_2) {
  ...
  function configure(aurelia, configCallback) {
    aurelia.container.registerHandler('ui-validator', function(container) {
      return container.get(ui_validation_1.UIValidationRenderer);
    });
    ...
    aurelia_validation_1.ValidationRules
      .customRule('phone', function(value, obj) {
        return value === null || value === undefined || PhoneLib.isValid(value);
      }, '\${$displayName } is not a valid phone number.');
    aurelia_validation_1.ValidationRules
      .customRule('integer', function(value, obj, min, max) {
        return value === null || value === undefined || Number.isInteger(value) && value >= (min || Number.MIN_VALUE) && value <= (max || Number.MAX_VALUE);
      }, '\${$displayName} must be an integer value between \${$config.min || "MIN_VALUE"} and \${$config.max || "MAX_VALUE"}.', function(min, max) {
        return ({
          min: min,
          max: max
        });
    ...
    aurelia_validation_1.ValidationRules
      .customRule('decimal', function(value, obj, min, max) {
        return value === null || value === undefined || Math.floor(value % 1) === 0 && value >= (min || Number.MIN_VALUE) && value <= (max || Number.MAX_VALUE);
      }, '\${$displayName} must be a decimal value between \${$config.min || "MIN_VALUE"} and \${$config.max || "MAX_VALUE"}.', function(min, max) {
        return ({
          min: min,
          max: max
        });
